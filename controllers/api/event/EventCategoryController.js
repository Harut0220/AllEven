...
class EventCategoryController {
  ...
  index = async (req, res) => {
    ...
    let categories =
      user && user.event_favorite_categories.length && userRole == "USER"
        ? await this.EventCategoryService.getByUser(
  ...
  store = async (req, res) => {
    ...
    let category = await this.EventCategoryService.store({
      name,
      description,
      owner: user.id,
    });
    ...
    return res.json({
      status: category,
      message: "Админ будет подтверждать ваш запрос",
    });
  };
}
