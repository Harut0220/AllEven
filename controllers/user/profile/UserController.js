...
class UserController {
  ...
  destroy = async (req, res) => {
    ...
    setTimeout(async () => {
      ...
      const companiesDbforParticipants = await companyModel.find();
      ...
      for await (const company of companiesDbforParticipants) {
      ...
      for await (const like of EventAnswerLikeDb) {
        await EventCommentAnswer.findByIdAndUpdate(like.answerId, {
          $pull: { likes: like._id },
        });
      }
      ...
      if (eventsDb.length) {
        for await (const eventData of eventsDb) {
          ...
          await EventComment.deleteMany({ event: eventData._id });
          await EventCommentLikes.deleteMany({ eventId: eventData._id });
          await EventFavorites.deleteMany({ eventId: eventData._id });
          await EventView.deleteMany({ eventId: eventData._id });
          await EventRating.deleteMany({ event: eventData._id });
          await EventImpressionImages.deleteMany({ event: eventData._id });
          await EventParticipantsSpot.deleteMany({ eventId: eventData._id });
          await EventParticipants.deleteMany({ eventId: eventData._id });
          await event.remove();
      ...
      for await (const companyAnswerLike of CompanyAnswerLikeDb) {
        await companyCommentAnswer.findByIdAndUpdate(
          companyAnswerLike.answerId,
          {
            $pull: { likes: companyAnswerLike._id },
          }
        );
      }
      ...
      for await (const companyAnswer of CompanyAnswerDb) {
        await companyComment.findByIdAndUpdate(companyAnswer.commentId, {
          $pull: { answer: companyAnswer._id },
        });
      }
      ...
      if (meetingsDb.length) {
        for await (const meeting of meetingsDb) {
          ...
          await meetingComment.deleteMany({ meetingId: meeting._id });
          await meetingImages.deleteMany({ meetingId: meeting._id });
          await meetingLikes.deleteMany({ meetingId: meeting._id });
          await meetingFavorit.deleteMany({ meetingId: meeting._id });
          await meetingParticipantSpot.deleteMany({ meetingId: meeting._id });
          await meetingView.deleteMany({ meetingId: meeting._id });
          await meetingRating.deleteMany({ meetingId: meeting._id });
          await meetingParticipant.deleteMany({ meetingId: meeting._id });
          await meetingImpressionImage.deleteMany({ meetingId: meeting._id });
          await meetingVerify.deleteMany({ user: user1.id });
          await meetingDoc.remove();
          console.log("Meetings and all related data deleted successfully");
        }
      }
