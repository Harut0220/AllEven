...
class NotificationService {
  ...
  allAggregate = async (params = {}) => {
    ...
    for (const notification of notifications) {
      notification.date_time = moment(
        new Date(notification.date_time),
    ...
    for (let n = 0; n < nots.length; n++) {
      ...
      if (range > 0 && range <= 600000 && nots[n].status == 1) {
        ...
        for (let u = 0; u < users.length; u++) {
          let f = await this.FeedbackService.store({
            to: users[u]._id,
            message: nots[n].message,
          });
